import numpy as np
'''
a = np.array([[4, -2, 1],[3, 6, -4],[2, 1, 8]])
b = np.array([20, -30, 40])
x = np.linalg.solve(a, b)
'''
n = 4
a = [-2, -2, -2]#in the first eqation there is no a that is why its a 0
b = [4, 6, 6, 8]
c = [-2, -2, -2]
d = [5, 0, 0, 0]

#xj+ej*xj+1=fj
#ej+1 = (cj+1)/(bj+1 - aj+1*ej)
#fj+1 = (bj+1 - aj+1*fj)/(bj+1 - aj+1*ej)
#all for (ai*ei-1 - bi)*xi = ai*fi-1 - bi

e = [0, 0, 0, 0]
f = [0, 0, 0, 0]
x = [0, 0, 0, 0]

e[0] = c[0] / b[0]
f[0] = d[0] / b[0]

for i in range(1,n-1):#it will go from [1] to [n-1]
    e[i] = c[i] / (b[i] - a[i-1]*e[i-1])
    f[i] = (d[i] - a[i-1]*f[i-1]) / (b[i] - a[i-1]*e[i-1])
print(e,f)


x[n-1] = (d[n-1] - a[n-2]*f[n-2]) / (b[n-1] - a[n-2]*e[n-2])

for i in range(n-2, -1, -1):#goes till 0
    x[i] = f[i] - e[i]*x[i+1]
print('x=', [round(i,n+1) for i in x])
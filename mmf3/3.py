...
def y(t):
    return yo2 + C*math.exp(D*t) - (yo1 + A*math.cos(B*t))

def dy(t):
    return 3*math.sin(3*t) + math.exp(t / 2)

def y3(t):
    return math.cos(t)
#establish an interval a and b
#the function will take in the interval and the tolerance
def fbisection(y,a,b,tolerance):
    ta = a
    tb = b
    if y(ta)*y(tb) < 0:
...
def fnewtrhapsmethod(func, funcderivetive, x, n):

    for i in range(1,n):#how many times we search for a closer aproxiamtion
        xo = x - (func(x)/funcderivetive(x))
...
def generalnewtrhapsmethod(func, funcderivetive, x, n):
...
print(fbisection(y,2,4,0.004))
print(fnewtrhapsmethod(y, dy,2.5,50))